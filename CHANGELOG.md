...
- Gas bills without a separately labelled emission date now use the end of
  their bill-provided consumption period for status calculations, rather than
  leaving the service permanently in a problem state.
...
- Confirmed that common-expenses sensors are populated from the add-on's
  structured `sections` response and added regression coverage for the
  `coe_administraciones` response shape.
- Added compatibility fallback to values from
  `meta.matched_sections.*.line_results` when the corresponding canonical
  section value is absent or null. Null extraction results remain missing so
  the existing raw OCR and internal-extractor fallbacks can run.
...
- Structured add-on currency cells (such as `$ 122.060`) are now parsed as
  amounts instead of zero, and renamed or nested template fields remain
  compatible with the Gastos Comunes sensors.
...
- Added Bearer authentication to add-on status, health, and OCR requests.
